...
const sampleCSV = `Company Name,Contact Name,Email,Trade
ABC Plumbing,Dave Smith,dave@abcplumbing.com.au,Plumbing
XYZ Electrical,Mike Jones,mike@xyzelectrical.com.au,Electrical
SEQ Concreting,Paul Brown,paul@seqconcreting.com.au,Concreting
Brisbane Frames,Tom White,tom@brisframes.com.au,Framing
North Build Co,Steve Green,,Labourer`;
...
export default function BulkInvite() {
  ...
  const parseCSV = (text: string) => {
    ...
    const data = lines.slice(1).map((line, i) => {
      const [company, contact, email, trade] = line.split(",").map((s) => s.trim());
  ...
  const inputStyle: React.CSSProperties = {
    ...
    fontSize: "12px", color: "#111", borderRadius: "2px", fontFamily: "Roboto, sans-serif",
  };
    ...
    <div>
      <div style={{ padding: "14px 32px", borderBottom: "1px solid #d0d0d0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          ...
          <div style={{ fontSize: "12px", color: "#333", marginTop: "2px" }}>Upload a CSV to invite multiple contractors at once</div>
        </div>
        <a href="/" style={{ fontSize: "12px", color: "#333", textDecoration: "none" }}>← Back to dashboard</a>
      </div>
        ...
        {step === "upload" && (
          <div>
            <div style={{ marginBottom: "24px" }}>
              <div style={{ fontSize: "12px", fontWeight: 500, color: "#444", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: "12px" }}>Step 1 — upload your CSV file</div>
              <div
                onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
                onDragLeave={() => setDragOver(false)}
                onDrop={handleDrop}
                style={{ border: `2px dashed ${dragOver ? "#111" : "#d0d0d0"}`, borderRadius: "2px", padding: "48px 32px", textAlign: "center", background: dragOver ? "#fafafa" : "#fff", transition: "border-color 0.15s" }}
              >
                <div style={{ fontSize: "14px", fontWeight: 500, color: "#111", marginBottom: "8px" }}>Drop your CSV here</div>
                <div style={{ fontSize: "12px", color: "#333", marginBottom: "16px" }}>or click to browse</div>
                <label style={{ padding: "8px 20px", background: "#111", color: "#fff", border: "none", borderRadius: "2px", fontSize: "12px", fontWeight: 500, cursor: "pointer", fontFamily: "Roboto, sans-serif" }}>
                  Choose file
            ...
            <div style={{ marginBottom: "24px" }}>
              <div style={{ fontSize: "12px", fontWeight: 500, color: "#444", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: "10px" }}>CSV format required</div>
              <div style={{ border: "1px solid #d0d0d0", borderRadius: "2px", overflow: "hidden" }}>
                <div style={{ padding: "8px 14px", background: "#fafafa", borderBottom: "1px solid #d0d0d0", fontSize: "11px", color: "#333" }}>Required columns — Company Name, Contact Name, Email, Trade</div>
            ...
            <div style={{ textAlign: "center" }}>
              <button
                onClick={() => parseCSV(sampleCSV)}
                style={{ fontSize: "12px", color: "#333", background: "none", border: "none", cursor: "pointer", textDecoration: "underline", fontFamily: "Roboto, sans-serif" }}
              >
        ...
        {step === "review" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
              <div style={{ fontSize: "12px", fontWeight: 500, color: "#444", textTransform: "uppercase", letterSpacing: ".08em" }}>Step 2 — review and fix errors</div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                <thead>
                  <tr style={{ background: "#fafafa" }}>
                    <th style={{ width: "24%", fontSize: "11px", fontWeight: 500, color: "#333", textAlign: "left", padding: "8px 12px", borderBottom: "1px solid #d0d0d0" }}>Company</th>
                    <th style={{ width: "20%", fontSize: "11px", fontWeight: 500, color: "#333", textAlign: "left", padding: "8px 12px", borderBottom: "1px solid #d0d0d0" }}>Contact</th>
                    <th style={{ width: "28%", fontSize: "11px", fontWeight: 500, color: "#333", textAlign: "left", padding: "8px 12px", borderBottom: "1px solid #d0d0d0" }}>Email</th>
                    <th style={{ width: "16%", fontSize: "11px", fontWeight: 500, color: "#333", textAlign: "left", padding: "8px 12px", borderBottom: "1px solid #d0d0d0" }}>Trade</th>
                    <th style={{ width: "12%", fontSize: "11px", fontWeight: 500, color: "#333", textAlign: "left", padding: "8px 12px", borderBottom: "1px solid #d0d0d0" }}></th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.id} style={{ borderBottom: "1px solid #ebebeb", background: row.status === "error" ? "#fff8f8" : "#fff" }}>
                      ...
                      <td style={{ padding: "8px 12px" }}>
                        ...
                        />
                        {row.error && <div style={{ fontSize: "12px", color: "#c0392b", marginTop: "2px" }}>{row.error}</div>}
                      </td>
                      <td style={{ padding: "8px 12px" }}>
                        <input value={row.trade} onChange={(e) => updateRow(row.id, { trade: e.target.value })} style={inputStyle} />
                      </td>
                      <td style={{ padding: "8px 12px", textAlign: "center" }}>
                        <span onClick={() => removeRow(row.id)} style={{ fontSize: "14px", color: "#444", cursor: "pointer" }}>×</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div style={{ marginBottom: "20px" }}>
              <div style={{ fontSize: "12px", fontWeight: 500, color: "#444", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: "10px" }}>Vetting for which sites</div>
              <div style={{ border: "1px solid #d0d0d0", borderRadius: "2px", overflow: "hidden" }}>
                {sites.map((s, i) => (
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <button onClick={() => { setStep("upload"); setRows([]); }} style={{ padding: "9px 20px", background: "#fff", color: "#111", border: "1px solid #d0d0d0", borderRadius: "2px", fontSize: "13px", cursor: "pointer", fontFamily: "Roboto, sans-serif" }}>
                ← Start over
              </button>
              <button
                onClick={sendInvites}
                disabled={readyCount === 0 || selectedSites.length === 0}
                style={{ padding: "9px 20px", background: readyCount === 0 || selectedSites.length === 0 ? "#aaa" : "#111", color: "#fff", border: "none", borderRadius: "2px", fontSize: "13px", fontWeight: 500, cursor: readyCount === 0 || selectedSites.length === 0 ? "not-allowed" : "pointer", fontFamily: "Roboto, sans-serif" }}
              >
        ...
        {step === "sent" && (
          <div style={{ textAlign: "center", padding: "48px 0" }}>
            ...
            <div style={{ border: "1px solid #ebebeb", borderRadius: "2px", overflow: "hidden", display: "inline-block", textAlign: "left", marginBottom: "24px", minWidth: "300px" }}>
              <div style={{ padding: "8px 14px", background: "#fafafa", borderBottom: "1px solid #ebebeb", fontSize: "12px", fontWeight: 500, color: "#444", textTransform: "uppercase", letterSpacing: ".07em" }}>Invites sent to</div>
              {rows.filter((r) => r.status === "sent").map((r) => (
                <div key={r.id} style={{ padding: "9px 14px", borderBottom: "1px solid #ebebeb", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <div style={{ fontSize: "11px", color: "#333", marginTop: "1px" }}>{r.email}</div>
                  </div>
            ...
            <div style={{ display: "flex", gap: "10px", justifyContent: "center" }}>
              <a href="/" style={{ padding: "9px 20px", background: "#111", color: "#fff", border: "none", borderRadius: "2px", fontSize: "13px", fontWeight: 500, cursor: "pointer", fontFamily: "Roboto, sans-serif", textDecoration: "none" }}>Back to dashboard</a>
              <button onClick={() => { setStep("upload"); setRows([]); setSelectedSites([]); }} style={{ padding: "9px 20px", background: "#fff", color: "#111", border: "1px solid #d0d0d0", borderRadius: "2px", fontSize: "13px", cursor: "pointer", fontFamily: "Roboto, sans-serif" }}>Send another batch</button>
            </div>
          </div>
        )}
      </div>
